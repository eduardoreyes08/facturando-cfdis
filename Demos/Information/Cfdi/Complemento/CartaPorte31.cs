...
{
  internal static class CartaPorte31
  {
    ...
    private static void RegimenesAduaneros(RegimenesAduaneros data)
    {
      if (data.Count == 0)
        return;
      ...
      {
        RegimenAduaneroCcp regimen = data[i];

        string title = $"CARTA PORTE - REGIMEN ADUANERO CCP - {i + 1}";
        Utils.ShowTitle(title);
        Utils.ShowField("RegimenAduanero", regimen.RegimenAduanero);
      }
    }
    ...
    private static void TransporteMaritimo(TransporteMaritimo data)
    {
      ...
      {
        Contenedor contenedor = data.Contenedores[i];

        string title = $"{Title} - CONTENEDOR {i + 1}";

        Utils.ShowTitle(title);
        Utils.ShowField("TipoContenedor       ", contenedor.TipoContenedor);
        Utils.ShowField("MatriculaContenedor  ", contenedor.MatriculaContenedor);
        Utils.ShowField("NumeroPrecinto       ", contenedor.NumeroPrecinto);
        Utils.ShowField("IdCcpRelacionado     ", contenedor.IdCcpRelacionado);
        Utils.ShowField("PlacaVmccp           ", contenedor.PlacaVmccp);
        Utils.ShowField("FechaCertificacionCcp", contenedor.FechaCertificacionCcp);

        for (int j = 0; j < contenedor.RemolquesCcp.Count; j++)
        {
          Utils.ShowTitle($"{title} - REMOLQUE CCP {j + 1}");
          Utils.ShowField("SubTipoRemolqueCcp", contenedor.RemolquesCcp[j].SubTipoRemolqueCcp);
          Utils.ShowField("PlacaCcp          ", contenedor.RemolquesCcp[j].PlacaCcp);
        }
      }
    }
